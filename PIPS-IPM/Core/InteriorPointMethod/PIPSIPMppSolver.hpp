...
class PIPSIPMppSolver : public Solver {
  public:
    PIPSIPMppSolver(DistributedFactory &factory, Problem &problem, InteriorPointMethodType interior_point_method_type,
    ...
    static double predicted_reduction(Problem &problem, Variables &iterate, Variables &direction, double mu,
                                      double step_length);
  ...
  protected:
    ...
    /** the i-th entry of this array contains the minimum value of phi encountered
     * by the algorithm on or before iteration i */
    ...
    std::pair<double, double> compute_unscaled_gap_and_residual_norm(const Problem &problem,
