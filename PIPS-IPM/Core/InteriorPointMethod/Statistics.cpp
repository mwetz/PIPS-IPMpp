...
Statistics::Statistics(Scaler *scaler)
    : scaler{scaler}, mpi_comm{MPI_COMM_WORLD}, rank{PIPS_MPIgetRank(mpi_comm)}, global_rank{rank} {}

Statistics::Statistics(const DistributedFactory &factory, const Scaler *scaler)
    : scaler{scaler}, mpi_comm{factory.tree->getCommWorkers()}, rank{PIPS_MPIgetRank(mpi_comm)},
      global_rank{PIPS_MPIgetRank()} {}

void Statistics::print(const Problem &problem, const Variables &variables, const Residuals &residuals, double dnorm,
                       double alpha, int i, double mu, TerminationStatus status_code, int level) const {
    Statistics::print(problem, variables, residuals, dnorm, alpha, -1.0, i, mu, status_code, level);
}

void Statistics::print(const Problem &problem, const Variables &variables, const Residuals &residuals, double dnorm,
                       double alpha_primal, double alpha_dual, int i, double mu, TerminationStatus status_code,
                       int level) const {
