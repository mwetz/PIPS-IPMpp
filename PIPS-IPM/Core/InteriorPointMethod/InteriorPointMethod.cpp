...
InteriorPointMethod::InteriorPointMethod(DistributedFactory &factory, Problem &problem, double dnorm,
                                         const Scaler *scaler)
    : dnorm(dnorm), corrector_step(factory.make_variables(problem)),
      ...
      temp_step(factory.make_variables(problem)), statistics(factory, scaler), bicgstab_skipped(false),
      bicgstab_converged(true), bigcstab_norm_res_rel(0.), bicg_iterations(0),
      dynamic_corrector_schedule(pipsipmpp_options::get_bool_parameter("GONDZIO_STOCH_USE_DYNAMIC_CORRECTOR_SCHEDULE")),
      additional_correctors_small_comp_pairs(
          pipsipmpp_options::get_bool_parameter("GONDZIO_STOCH_ADDITIONAL_CORRECTORS_SMALL_VARS")),
      ...
      number_small_correctors(0), maximum_correctors(options::get_int_parameter("GONDZIO_MAX_CORRECTORS")),
      number_gondzio_corrections(0), step_factor0(0.3), step_factor1(1.5), acceptance_tolerance(0.01), beta_min(0.1),
      beta_max(10), dynamic_bicg_tol(pipsipmpp_options::get_bool_parameter("OUTER_BICG_DYNAMIC_TOL")), tsig(3.),
...
PrimalInteriorPointMethod::PrimalInteriorPointMethod(DistributedFactory &factory, Problem &problem, double dnorm,
                                                     const Scaler *scaler)
    : InteriorPointMethod(factory, problem, dnorm, scaler), primal_step_length(1.) {
    this->set_BiCGStab_tolerance(-1);
}

PrimalDualInteriorPointMethod::PrimalDualInteriorPointMethod(DistributedFactory &factory, Problem &problem,
                                                             double dnorm, const Scaler *scaler)
    : InteriorPointMethod(factory, problem, dnorm, scaler), primal_step_length(1.), dual_step_length(1.) {
    this->set_BiCGStab_tolerance(-1);
}

bool InteriorPointMethod::compute_predictor_step(const Variables &current_iterate, Residuals &residuals,
...
void PrimalInteriorPointMethod::compute_fraction_to_boundary(const Variables &iterate, const Variables &step,
                                                             double fraction) {
    this->primal_step_length = iterate.fraction_to_boundary(step, fraction);
}

void PrimalDualInteriorPointMethod::compute_fraction_to_boundary(const Variables &iterate, const Variables &step,
                                                                 double fraction) {
    // TODO: use fraction
    std::tie(this->primal_step_length, this->dual_step_length) = iterate.stepbound_pd(step);
}

void PrimalInteriorPointMethod::compute_corrector_step(const Problem &problem, Variables &current_iterate,
                                                       Residuals &residuals, Variables &step,
    ...
    std::tie(alpha_candidate, weight_candidate) =
        calculate_alpha_weight_candidate(current_iterate, step, *corrector_step, this->primal_step_length);
    assert(weight_candidate >= 0. && weight_candidate <= 1.);

    step.add(*corrector_step, weight_candidate);

    // prepare for Gondzio corrector loop: zero out the corrector_residuals
    // structure:
    corrector_residuals->clear_linear_residuals();

    // Gondzio correction loop:
    this->gondzio_correction_loop(current_iterate, residuals, step, linear_system, iteration, sigma, mu, small_corr,
                                  numerical_troubles);

    // We've finally decided on a step direction, now calculate the length using
    // Mehrotra's heuristic
    this->mehrotra_step_length(current_iterate, step);

    // if we encountered numerical troubles while computing the step, enter a
    // probing round
...
double PrimalInteriorPointMethod::compute_centering_parameter(const Variables &iterate, const Variables &step) {
...
double PrimalDualInteriorPointMethod::compute_centering_parameter(const Variables &iterate, const Variables &step) {
...
void PrimalDualInteriorPointMethod::compute_corrector_step(const Problem &problem, Variables &current_iterate,
                                                           Residuals &residuals, Variables &step,
                                                           AbstractLinearSystem &linear_system, int iteration,
                                                           bool small_corr) {
    // calculate centering parameter
    ...
    std::tie(alpha_primal_candidate, alpha_dual_candidate, weight_primal_candidate, weight_dual_candidate) =
        calculate_alpha_pd_weight_candidate(current_iterate, step, *corrector_step, this->primal_step_length,
                                            this->dual_step_length);
    ...
    // if we encountered numerical troubles while computing the step check enter a
    // probing round
    if (numerical_troubles) {
        if (precond_decreased)
            precond_decreased = decrease_preconditioner_impact(&linear_system);
    ...
    if (PIPS_MPIgetRank() == 0) {
        ...
        std::cout << "Direction has norm: " << step_norm << ", ";
        std::cout << "including primals (" << step_primals_norm << ") and slacks (" << step_slacks_norm << ")\n";
        std::cout << "Step lengths: " << initial_primal_step_length << " (primal), " << initial_dual_step_length
                  << " (dual)\n";
    }
...
void PrimalDualInteriorPointMethod::gondzio_correction_loop(Variables &iterate, Residuals &residuals, Variables &step,
                                                            AbstractLinearSystem &linear_system, int iteration,
                                                            double sigma, double mu, bool &small_corr,
    ...
    while (number_gondzio_corrections < maximum_correctors && number_small_correctors < max_additional_correctors &&
        ...
        PIPSdebugMessage("corrector loop: %d alpha_primal: %f alpha_dual %f \n", number_gondzio_corrections,
                         this->alpha_primal_candidate, this->alpha_dual_candidate);
        ...
        if (numerical_troubles) {
            if (!was_small_corr && small_corr)
                ...
                // exit corrector loop if small correctors have already been tried or
                // are not allowed
                break;
        }

        // calculate weighted predictor-corrector step
        std::tie(alpha_primal_enhanced, alpha_dual_enhanced, weight_primal_candidate, weight_dual_candidate) =
            calculate_alpha_pd_weight_candidate(iterate, step, *corrector_step, alpha_p_target, alpha_dual_target);
        ...
        /* if not done yet because correctors were not good enough - try a small
           corrector if enabled */
        else if (additional_correctors_small_comp_pairs && !small_corr && iteration >= first_iter_small_correctors) {
            if (this->alpha_primal_candidate < max_alpha_small_correctors ||
                this->alpha_dual_candidate < max_alpha_small_correctors) {
                // try and center small pairs
                small_corr = true;
                if (my_rank == 0) {
                    std::cout << "Switching to small corrector " << std::endl;
                    std::cout << "Alpha when switching: " << this->alpha_primal_candidate << " "
                              << this->alpha_dual_candidate << std::endl;
                }
            } else
                // exit Gondzio correction loop
                break;
...
void PrimalInteriorPointMethod::gondzio_correction_loop(Variables &iterate, Residuals &residuals, Variables &step,
                                                        AbstractLinearSystem &linear_system, int iteration,
                                                        double sigma, double mu, bool &small_corr,
                                                        bool &numerical_troubles) {
    const int my_rank = PIPS_MPIgetRank(MPI_COMM_WORLD);
    ...
    while (number_gondzio_corrections < maximum_correctors && number_small_correctors < max_additional_correctors &&
        ...
        // calculate weighted predictor-corrector step
        auto [alpha_enhanced, weight_candidate] =
            calculate_alpha_weight_candidate(iterate, step, *corrector_step, alpha_target);
...
void InteriorPointMethod::compute_gondzio_corrector(const Variables &iterate, AbstractLinearSystem &linear_system,
                                                    double rmin, double rmax, bool small_corr) {
    ...
    linear_system.solve(iterate, *corrector_residuals,
                        *corrector_step); // corrector_step is now delta_m
}

std::pair<double, double> InteriorPointMethod::calculate_alpha_weight_candidate(const Variables &iterate,
                                                                                const Variables &predictor_step,
                                                                                const Variables &corrector_step,
                                                                                double alpha_predictor) {
    assert(temp_step);
...
InteriorPointMethod::calculate_alpha_pd_weight_candidate(const Variables &iterate, const Variables &predictor_step,
                                                         const Variables &corrector_step, double alpha_primal,
                                                         double alpha_dual) {
...
double InteriorPointMethod::compute_probing_factor(const Problem &problem, const Variables &iterate,
                                                   Residuals &residuals, Variables &step) {
...
void PrimalInteriorPointMethod::do_probing(const Problem &problem, const Variables &iterate, Residuals &residuals,
                                           Variables &step) {
...
void PrimalDualInteriorPointMethod::do_probing(const Problem &problem, const Variables &iterate, Residuals &residuals,
                                               Variables &step) {
    const double factor = InteriorPointMethod::compute_probing_factor(problem, iterate, residuals, step);
    this->primal_step_length = factor * this->primal_step_length;
...
bool InteriorPointMethod::is_poor_step(bool &pure_centering_step, bool precond_decreased, double alpha_max) const {
    ...
    if (!pure_centering_step && alpha_max < mutol * 1e-2) {
        ...
        if (my_rank == 0)
            std::cout << "poor step computed - refactorization with decreased "
                         "preconditioning\n";
        return true;
    }
...
void PrimalInteriorPointMethod::compute_probing_step(Variables &probing_step, const Variables &iterate,
                                                     const Variables &step) const {
    probing_step.copy(iterate);
    probing_step.add(step, this->primal_step_length);
}

void PrimalDualInteriorPointMethod::compute_probing_step(Variables &probing_step, const Variables &iterate,
                                                         const Variables &step) const {
    probing_step.copy(iterate);
    probing_step.add(step, this->primal_step_length, this->dual_step_length);
}

/* when numerical troubles occurred we only allow controlled steps that worsen
 * the residuals and mu by at most a factor of 10 */
double InteriorPointMethod::compute_step_factor_probing(double resids_norm_last, double resids_norm_probing,
                                                        double mu_last, double mu_probing) {
...
void InteriorPointMethod::check_numerical_troubles(Residuals *residuals, bool &numerical_troubles,
                                                   bool &small_corr) const {
...
void PrimalInteriorPointMethod::print_statistics(const Problem &problem, const Variables &iterate,
                                                 const Residuals &residuals, int i, double mu,
...
void PrimalDualInteriorPointMethod::print_statistics(const Problem &problem, const Variables &iterate,
                                                     const Residuals &residuals, int i, double mu,
                                                     TerminationStatus stop_code, int level) {
    statistics.print(problem, iterate, residuals, dnorm, this->primal_step_length, this->dual_step_length, i, mu,
                     stop_code, level);
}
...
void PrimalInteriorPointMethod::mehrotra_step_length(Variables &iterate, Variables &step) {
    ...
    const double maximum_step_length =
        iterate.find_blocking(step, primalValue, primalStep, dualValue, dualStep, firstOrSecond);
...
void PrimalDualInteriorPointMethod::mehrotra_step_length(Variables &iterate, Variables &step) {
    ...
    iterate.find_blocking(step, primalValue_p, primalStep_p, dualValue_p, dualStep_p, primalValue_d, primalStep_d,
...
std::unique_ptr<InteriorPointMethod> MehrotraFactory::create(DistributedFactory &factory, Problem &problem,
                                                             double dnorm,
