...
PIPSIPMppInterface::PIPSIPMppInterface(DistributedInputTree *tree, InteriorPointMethodType mehrotra_heuristic,
                                       MPI_Comm comm, ScalerType scaler_type, PresolverType presolver_type,
                                       const std::string &settings)
    : comm(comm), my_rank(PIPS_MPIgetRank(comm)) {
    ...
    if (presolver_type != PresolverType::NONE) {
        ...
        presolver = preprocess_factory->make_presolver(*factory->tree, original_problem.get(), presolver_type,
                                                       postsolver.get());
   ...
   myfile.close();
    ...
    if (pipsipmpp_options::get_bool_parameter("HIERARCHICAL")) {
        ...
        presolved_problem.reset(dynamic_cast<DistributedProblem *>(
            factory->switchToHierarchicalData(dynamic_cast<DistributedProblem *>(presolved_problem.release()))));
...
TerminationStatus PIPSIPMppInterface::run() {
    ...
    if (my_rank == 0) {
        ...
        if (!pipsipmpp_options::get_bool_parameter("HIERARCHICAL")) {
            std::cout << "1st stage " << qp_presolved_problem.getLocalnx() << " variables, "
                      << qp_presolved_problem.getLocalmy() << " equality constraints, "
                      << qp_presolved_problem.getLocalmz() << " inequality constraints.\n";
            ...
            if (nscens) {
                std::cout << "2nd stage " << qp_presolved_problem.children[0]->getLocalnx() << " variables, "
                          << qp_presolved_problem.children[0]->getLocalmy() << " equality constraints, "
                          << qp_presolved_problem.children[0]->getLocalmz() << " inequality constraints.\n";

                std::cout << nscens << " scenarios."
                          << "\n";
                std::cout << "Total "
                          << qp_presolved_problem.getLocalnx() + nscens * qp_presolved_problem.children[0]->getLocalnx()
                          << " variables, "
                          << qp_presolved_problem.getLocalmy() + nscens * qp_presolved_problem.children[0]->getLocalmy()
                          << " equality constraints, "
                          << qp_presolved_problem.getLocalmz() + nscens * qp_presolved_problem.children[0]->getLocalmz()
...
TerminationStatus PIPSIPMppInterface::termination_status() const { return result; }

int PIPSIPMppInterface::n_iterations() const {
    if (!ran_solver)
        throw std::logic_error(
            "Must call run() and start solution process before trying to retrieve the iteration count!");
...
double PIPSIPMppInterface::getObjective() {
    ...
    if (!ran_solver)
        throw std::logic_error(
            "Must call run() and start solution process before trying to retrieve original solution");
...
void PIPSIPMppInterface::getVarsUnscaledUnperm() {
    ...
    if (!ran_solver)
        throw std::logic_error(
            "Must call run() and start solution process before trying to retrieve unscaled unpermuted solution");
    if (scaler) {
        std::unique_ptr<DistributedVariables> unscaled_vars =
            std::make_unique<DistributedVariables>(dynamic_cast<const DistributedVariables &>(*variables));
...
void PIPSIPMppInterface::getResidsUnscaledUnperm() {
    ...
    if (!ran_solver)
        throw std::logic_error(
            "Must call run() and start solution process before trying to retrieve unscaled unpermuted residuals");
    if (scaler) {
        std::unique_ptr<DistributedResiduals> unscaled_residuals =
            std::make_unique<DistributedResiduals>(dynamic_cast<const DistributedResiduals &>(*residuals));
...
std::vector<double>
PIPSIPMppInterface::gatherFromSolution(std::unique_ptr<Vector<double>> Variables::*member_to_gather) {
    ...
    if (!postsolver)
        vec = dynamic_cast<const DistributedVector<double> &>(*(*unscaleUnpermNotHierVars.*member_to_gather))
                  .gatherStochVector();
    else
        vec = dynamic_cast<const DistributedVector<double> &>(*(*postsolved_variables.*member_to_gather))
                  .gatherStochVector();
...
std::vector<double>
PIPSIPMppInterface::gatherFromResiduals(std::unique_ptr<Vector<double>> Residuals::*member_to_gather) {
    ...
    std::vector<double> vec;
    if (!postsolver)
        vec = dynamic_cast<const DistributedVector<double> &>(*(*unscaleUnpermNotHierResids.*member_to_gather))
                  .gatherStochVector();
    else
        vec =
            dynamic_cast<const DistributedVector<double> &>(*(*postsolvedResids.*member_to_gather)).gatherStochVector();
...
std::vector<double> PIPSIPMppInterface::gatherDualSolutionVarBounds() {
    ...
    std::transform(duals_varbounds_low.begin(), duals_varbounds_low.end(), duals_varbounds_upp.begin(),
                   std::back_inserter(duals_varbounds), std::minus<>());
...
std::vector<double> PIPSIPMppInterface::gatherEqualityConsValues() {
    ...
    DistributedVector<double> *eq_vals =
        (postsolved_variables == nullptr)
...
std::vector<double> PIPSIPMppInterface::gatherInequalityConsValues() {
    ...
    DistributedVector<double> *ineq_vals =
        (postsolved_variables == nullptr)
...
std::vector<double> PIPSIPMppInterface::getFirstStagePrimalColSolution() const {
    auto const &v = dynamic_cast<const DenseVector<double> &>(
        *dynamic_cast<DistributedVector<double> const &>(*variables->primals).first);
...
std::vector<double> PIPSIPMppInterface::getSecondStagePrimalColSolution(int scen) const {
    auto const &v = dynamic_cast<const DenseVector<double> &>(
        *dynamic_cast<DistributedVector<double> const &>(*variables->primals).children[scen]->first);
    if (!v.length())
...
void PIPSIPMppInterface::allgatherBlocksizes(std::vector<unsigned int> &block_lengths_col,
    ...
    /// gather col lengths
    const auto &col_vec = presolver
                              ? dynamic_cast<const DistributedVector<double> &>(*original_problem->objective_gradient)
    ...
    /// gather row lengths
    const auto &row_A_vec = presolver
                                ? dynamic_cast<const DistributedVector<double> &>(*original_problem->equality_rhs)
                                : dynamic_cast<const DistributedVector<double> &>(*presolved_problem->equality_rhs);
    const auto &row_C_vec =
        presolver
            ? dynamic_cast<const DistributedVector<double> &>(*original_problem->inequality_upper_bound_indicators)
